...
def test_job_processing_duration_seconds_metric():
    ...
    assert metrics.job_processing_duration_seconds._name == "ocr_job_processing_duration_seconds"
...
def test_job_total_duration_seconds_metric():
    ...
    assert metrics.job_total_duration_seconds._name == "ocr_job_total_duration_seconds"
...
def test_job_queue_duration_seconds_metric():
    ...
    assert metrics.job_queue_duration_seconds._name == "ocr_job_queue_duration_seconds"
...
def test_active_jobs_metric():
    ...
    assert metrics.active_jobs._name == "ocr_active_jobs"
...
def test_document_size_bytes_metric():
    ...
    assert metrics.document_size_bytes._name == "ocr_document_size_bytes"
...
def test_document_pages_metric():
    ...
    assert metrics.document_pages._name == "ocr_document_pages"
...
def test_sync_ocr_requests_total_metric():
    ...
    assert "engine" in metrics.sync_ocr_requests_total._labelnames
    assert "status" in metrics.sync_ocr_requests_total._labelnames
...
def test_sync_ocr_duration_seconds_metric():
    ...
    assert "engine" in metrics.sync_ocr_duration_seconds._labelnames
...
def test_sync_ocr_timeouts_total_metric():
    ...
    assert "engine" in metrics.sync_ocr_timeouts_total._labelnames
...
def test_sync_ocr_file_size_bytes_metric():
    ...
    assert "engine" in metrics.sync_ocr_file_size_bytes._labelnames
...
def test_counter_increment():
    ...
    # Get initial value
    initial = metrics.jobs_created_total._value.get()

    # Increment
    metrics.jobs_created_total.inc()

    # Check incremented
    assert metrics.jobs_created_total._value.get() == initial + 1
...
def test_metric_naming_convention():
    ...
    metric_names = [
        metrics.jobs_created_total._name,
        metrics.jobs_completed_total._name,
        metrics.active_jobs._name,
        metrics.document_size_bytes._name,
    ]
...
def test_metrics_have_help_text():
    ...
    assert metrics.jobs_created_total._documentation
    assert metrics.jobs_completed_total._documentation
    assert metrics.active_jobs._documentation
    assert len(metrics.jobs_created_total._documentation) > 0
