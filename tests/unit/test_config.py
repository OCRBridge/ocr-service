...
def test_settings_defaults():
    ...
    # OCR Configuration
    assert settings.default_ocr_engine == "tesseract"
    assert settings.tesseract_lang == "eng"
    assert settings.tesseract_psm == 3
    assert settings.tesseract_oem == 1
...
def test_settings_ocr_engine_override(monkeypatch):
    """Test overriding default OCR engine."""
    monkeypatch.setenv("DEFAULT_OCR_ENGINE", "easyocr")

    settings = Settings()

    assert settings.default_ocr_engine == "easyocr"


def test_settings_tesseract_params_override(monkeypatch):
    """Test overriding Tesseract parameters."""
    monkeypatch.setenv("TESSERACT_LANG", "fra+deu")
    monkeypatch.setenv("TESSERACT_PSM", "6")
    monkeypatch.setenv("TESSERACT_OEM", "2")
    monkeypatch.setenv("PDF_DPI", "600")

    settings = Settings()

    assert settings.tesseract_lang == "fra+deu"
    assert settings.tesseract_psm == 6
    assert settings.tesseract_oem == 2
    assert settings.pdf_dpi == 600
