...
def test_registry_loads_v2_models():
    ...
    try:
        ...
        with patch("src.services.ocr.registry_v2.entry_points", mock_ep):
            ...
            # Check EasyOCR
            easy_model = registry.get_param_model("easyocr")
            assert easy_model is not None
            assert easy_model.__name__ == "EasyOCRParams"
            assert "languages" in easy_model.model_fields # Field from spec/v2 model
